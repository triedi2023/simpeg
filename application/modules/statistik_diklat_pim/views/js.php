<script type="text/javascript">
    $('body').on('submit', "form#formpencarian", function (e) {
        ...
        $.ajax({
            ...
            kdu5_id:$("select#search_kdu5").val(),jenis_keluaran:$("select#jenis_keluaran").val(),bentuk:$("input[name='bentuk']:checked"). val()},
            dataType: "html",
            cache: false
    ...
    $('body').on('click', "select#jenis_keluaran", function (e) {
        if ($(this).val() == 1) {
            $('div.viewbentuk').show();
        } else {
            $('div.viewbentuk').hide();
    ...
    $('body').on('click', "a.btnexport", function (e) {
        window.open($(this).attr('data-url')+'?trlokasi_id='+$("select#search_trlokasi_id").val()+'&kdu1_id='+$("select#search_kdu1").val()
        +'&kdu2_id='+$("select#search_kdu2").val()+'&kdu3_id='+$("select#search_kdu3").val()+'&kdu4_id='+$("select#search_kdu4").val()
        +'&kdu5_id='+$("select#search_kdu5").val()+'&jenis_keluaran='+$("select#jenis_keluaran").val()+'&bentuk='+$("input[name='bentuk']:checked"). val());
        return false;
    });
