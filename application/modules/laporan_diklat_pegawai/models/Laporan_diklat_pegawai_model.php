...
class Laporan_diklat_pegawai_model extends CI_Model {
    ...
    function get_data($lok='2',$kdu1='00',$kdu2='00',$kdu3='000',$kdu4='000',$kdu5='00') {
        ...
        $sql = "SELECT * FROM V_PERIODIK WHERE 1=1 $where ";
        ...
        $i = 0;
        if ($pokok):
            foreach ($pokok as $val) {
                $clause = array('TMPEGAWAI_ID' => $val['ID']);
                $data[$i] = $val;
                $data[$i]['dtl_jabatan'] = $this->get_data_detail('dtl_jabatan', $clause, 'tmt_jabatan', 'DESC');
                $data[$i]['diklat_pim'] = $this->get_data_detail('diklat_pim', $clause, 'thn_diklat', 'DESC');
                $data[$i]['diklat_fungsional'] = $this->get_data_detail('diklat_fungsional', $clause, 'thn_diklat', 'DESC');
                $data[$i]['diklat_teknis'] = $this->get_data_detail('diklat_teknis', $clause, 'thn_diklat', 'DESC');
                $data[$i]['diklat_lain'] = $this->get_data_detail('diklat_lain', $clause, 'thn_diklat', 'DESC');
                $data[$i]['pendidikan'] = $this->get_data_detail('pendidikan', $clause, 'thn_lulus', 'DESC');
                $i++;
            }
        endif;
    ...
    function get_data_detail($table = FAlSE, $clause = FALSE, $order = FALSE, $type_order = 'DESC') {
        if ($table == 'pendidikan') {
            ...
            $query = $this->db->query("select N_JABATAN,TO_CHAR(TMT_JABATAN,'DD/MM/YYYY') AS TMT_JABATAN2,TO_CHAR(TMT_ESELON,'DD/MM/YYYY') AS TMT_ESELON2 from TH_PEGAWAI_JABATAN TPJ where TMPEGAWAI_ID=? ORDER BY TMT_JABATAN DESC",[$clause['TMPEGAWAI_ID']]);
            return $query->result_array();
        }
    }
