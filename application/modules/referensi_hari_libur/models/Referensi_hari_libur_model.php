...
class Referensi_hari_libur_model extends CI_Model {

    private $tabel = "TR_HARI_LIBUR";
    private $column_order = array(null, 'JENIS_LIBUR', 'TGL_LIBUR', 'STATUS'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('jenis_libur')) {
            $this->db->where('TRJENISLIBUR_ID', $this->input->post('jenis_libur'));
        }
        if (isset($_POST['status']) && ($_POST['status'] === '0' || !empty($_POST['status']))) {
            $this->db->where('TR_HARI_LIBUR.STATUS', $this->input->post('status'));
        }

        $this->db->select("TR_HARI_LIBUR.ID,TR_HARI_LIBUR.STATUS,JENIS_LIBUR,TO_CHAR(TGL_LIBUR,'DD/MM/YYYY') AS TGL_LIBUR");
        $this->db->from($this->tabel);
        $this->db->join("TR_JENIS_LIBUR","TR_JENIS_LIBUR.ID=TR_HARI_LIBUR.TRJENISLIBUR_ID","LEFT");
        $i = 0;
