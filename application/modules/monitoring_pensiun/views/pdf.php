...
// content
$pdf->SetWidths(array(10, 47, 30, 30, 100, 30, 30));
$pdf->HeaderTable(['No', 'Nama / NIP', 'Gol / Ruang', 'Jabatan', 'Jabatan / a. TMT Jabatan', 'Masa Kerja (Tahun / Bulan)', 'TMT Pensiun']);
$t = 1;
foreach ($model as $val):
    $nama = ((!empty($val['GELAR_DEPAN'])) ? $val['GELAR_DEPAN'] . " " : "") . ($val['NAMA']) . ((!empty($val['GELAR_BLKG'])) ? ", " . $val['GELAR_BLKG'] : '');
    $pdf->Row([$t, $nama . "\n" . $val['NIPNEW'], ($val['TRSTATUSKEPEGAWAIAN_ID'] == '1') ? $val['PANGKAT']." (".$val['GOLONGAN_RUANG'].") " : $val['PANGKAT'],$val['KEL_JABATAN'],$val['N_JABATAN']."\n"."a. ".$val['TMT_JABATAN'],$val['MK_THN_TOTAL']." Tahun ".$val['MK_BLN_TOTAL']." Bulan",$val['TMT_PENSIUN_CHAR']]);
    $t++;
endforeach;
