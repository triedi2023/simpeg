...
class Monitoring_ak_fungsional extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('monitoring_ak_fungsional/monitoring_ak_fungsional_model', 'list_model'));
        $this->load->helper(array('form', 'app_helper'));
        $this->data['plugin_js'] = ['assets/plugins/select2/js/select2.full.min.js'];
        $this->data['custom_js'] = ['monitoring_ak_fungsional/js', 'system/unitkerja_form-horizontal_filter_js'];
        $this->data['title_utama'] = 'Monitoring Angka Kredit';
    }

    public function index() {
        $this->data['content'] = 'monitoring_ak_fungsional/index';
    ...
    public function pencarian_proses() {
        ...
        $this->data['model'] = $this->monitoring_ak_fungsional_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['struktur'] = $this->monitoring_ak_fungsional_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);

        $this->load->view("monitoring_ak_fungsional/_hasil", $this->data);
    }
    ...
    public function export_excel() {
        ...
        $model = $this->monitoring_ak_fungsional_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $struktur = $this->monitoring_ak_fungsional_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $jumlah = count($model);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Monitoring Angka Kredit Jabatan Fungsional');
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('B' . $judultabel, "Nama / NIP / Tgl Lahir")
                ->setCellValue('C' . $judultabel, "Gol / Ruang")
                ->setCellValue('D' . $judultabel, "Jabatan / a. TMT Jabatan")
                ->setCellValue('E' . $judultabel, "AK Minimal")
                ->setCellValue('F' . $judultabel, "AK Saat Ini");
        ...
        if ($model) {
            foreach ($model as $val) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('E' . $j, $val['AK_MINIMAL'])
                        ->setCellValue('F' . $j, $val['AK_SAAT_INI']);
        ...
        $objPHPExcel->getActiveSheet()->getHeaderFooter()->setOddHeader('&L&G&C&HDAFTAR MONITORING AK Jabatan Fungsional' . $this->config->item('instansi_panjang'));
    ...
    public function export_pdf() {
        ...
        $this->data['model'] = $this->monitoring_ak_fungsional_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['struktur'] = $this->monitoring_ak_fungsional_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['content'] = 'monitoring_ak_fungsional/pdf';
        $this->load->view('layouts/main', $this->data);
