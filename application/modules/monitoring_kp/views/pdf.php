...
$pdf->Cell(50,14,'Nama / NIP / Tgl Lahir / Umur',1,0,'C');
$pdf->Cell(105,14,'Jabatan / a. TMT Jabatan',1,0,'C');
$pdf->Cell(30,14,'Pendidikan',1,0,'C');
$pdf->Cell(50,7,'Gol. Ruang & TMT',1,0,'C');
$pdf->Cell(30,14,'Keterangan',1,0,'C');
$pdf->Cell(0,7,'',0,1);
$pdf->Cell(195,7,'',0,0);
$pdf->Cell(25,7,'a',1,0,'C');
$pdf->Cell(25,7,'b',1,0,'C');
$pdf->Ln();
$pdf->SetWidths(array(10, 50, 105, 30, 25, 25, 30));
$t = 1;
foreach ($model_reguler as $val):
    $nama = ((!empty($val['GELAR_DEPAN'])) ? $val['GELAR_DEPAN'] . " " : "") . ($val['NAMA']) . ((!empty($val['GELAR_BLKG'])) ? ", " . $val['GELAR_BLKG'] : '');
    $pdf->Row([$t,$nama."\n"."NIP. ".$val['NIPNEW']."\n".$val['TGL_LAHIR']."\n"."(".$val['UMUR'].")",$val['N_JABATAN']."\n"."a. ".$val['TMT_JABATAN'],$val['TINGKAT_PENDIDIKAN'],
    (isset($val['TRGOLONGAN']) ? $val['TRGOLONGAN'] : '')."\n".(isset($val['TMT_GOL_ASLI']) ? $val['TMT_GOL_ASLI'] : ''),(isset($val['GOLONGAN_LAMA']) ? $val['GOLONGAN_LAMA'] : '')."\n".(isset($val['NEXT_TMT_KP_CHAR']) ? $val['NEXT_TMT_KP_CHAR'] : ''),'']);
    $t++;
endforeach;
