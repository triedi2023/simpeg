...
class Monitoring_kp extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('monitoring_kp/monitoring_kp_model', 'list_model'));
        $this->load->helper(array('form', 'app_helper'));
        $this->data['plugin_js'] = ['assets/plugins/select2/js/select2.full.min.js'];
        $this->data['custom_js'] = ['monitoring_kp/js', 'system/unitkerja_form-horizontal_filter_js'];
        $this->data['title_utama'] = 'Monitoring Kenaikan Pangkat';
    }

    public function index() {
        $this->data['content'] = 'monitoring_kp/index';
    ...
    public function pencarian_proses() {
        ...
        $this->data['model_reguler'] = $this->monitoring_kp_model->get_data_st_reguler($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $this->data['model_pilihan'] = $this->monitoring_kp_model->get_data_st_pilihan($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $this->data['model_fungsional'] = $this->monitoring_kp_model->get_data_fungsional($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $this->data['struktur'] = $this->monitoring_kp_model->get_struktur($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5);
        
        $this->load->view("monitoring_kp/_hasil", $this->data);
    }
    ...
    public function export_excel() {
        ...
        $model_reguler = $this->monitoring_kp_model->get_data_st_reguler($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $model_pilihan = $this->monitoring_kp_model->get_data_st_pilihan($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $model_fungsional = $this->monitoring_kp_model->get_data_fungsional($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $struktur = $this->monitoring_kp_model->get_struktur($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5);
        $jumlah = count($model_reguler) + count($model_pilihan) + count($model_fungsional);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Proyeksi Kenaikan Pangkat Reguler');
        ...
        if ($model_reguler) {
            foreach ($model_reguler as $val) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('E' . $j, (isset($val['TRGOLONGAN']) ? $val['TRGOLONGAN'] : '')."\n".(isset($val['TMT_GOL_ASLI']) ? $val['TMT_GOL_ASLI'] : ''))
                        ->setCellValue('F' . $j, (isset($val['GOLONGAN_LAMA']) ? $val['GOLONGAN_LAMA'] : '')."\n".(isset($val['NEXT_TMT_KP_CHAR']) ? $val['NEXT_TMT_KP_CHAR'] : ''))
                        ->setCellValue('G' . $j, '');
        ...
        if ($model_fungsional) {
            foreach ($model_fungsional as $val) {
                $nama = ((!empty($val['GELAR_DEPAN'])) ? $val['GELAR_DEPAN'] . " " : "") . ($val['NAMA']) . ((!empty($val['GELAR_BLKG'])) ? ", " . $val['GELAR_BLKG'] : '');
    ...
    public function export_pdf() {
        ...
        $this->data['model_fungsional'] = $this->monitoring_kp_model->get_data_fungsional($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$bulan,$tahun);
        $this->data['struktur'] = $this->monitoring_kp_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['content'] = 'monitoring_kp/pdf';
