...
class Monitoring_kp_model extends CI_Model {
    ...
    function get_data_st_reguler($lok='2',$kdu1='00',$kdu2='00',$kdu3='000',$kdu4='000',$kdu5='00',$gol_pangkat="",$bulan="",$tahun='') {
        ...
        $sql = "SELECT * FROM TABLE(f_mon_kp_reguler('$param')) WHERE TRESELON_ID NOT IN ('13','17') $where  ORDER BY TMT_GOL_ASLI";
        $query = $this->db->query($sql,$condition);
        
        return $query->result_array();
    }
    
    function get_data_st_pilihan($lok='2',$kdu1='00',$kdu2='00',$kdu3='000',$kdu4='000',$kdu5='00',$gol_pangkat="",$bulan="",$tahun='') {
        $tahun_sekarang = date('Y');
        $bulan_sekarang = date('m');
        $point = '';
        $where = '';
        ...
        $sql = "SELECT SUBQUERY.*,
            ...
            FROM TM_PEGAWAI TP 
            LEFT JOIN V_PEGAWAI_PANGKAT_MUTAKHIR VPPM ON (VPPM.TMPEGAWAI_ID=TP.ID) LEFT JOIN TR_GOLONGAN TRG ON VPPM.TRGOLONGAN_ID=TRG.ID
            LEFT JOIN V_PEGAWAI_PENDIDIKAN_MUTAKHIR VPEM ON VPEM.TMPEGAWAI_ID=TP.ID LEFT JOIN TR_TINGKAT_PENDIDIKAN TTP ON TTP.ID = VPEM.TRTINGKATPENDIDIKAN_ID
            LEFT JOIN V_PEGAWAI_JABATAN_MUTAKHIR VPJM ON VPJM.TMPEGAWAI_ID=TP.ID LEFT JOIN TR_JABATAN TRJ ON TRJ.ID = VPJM.TRJABATAN_ID
    ...
    function get_data_fungsional($lok='2',$kdu1='00',$kdu2='00',$kdu3='000',$kdu4='000',$kdu5='00',$gol_pangkat="",$bulan="",$tahun='') {
        $tahun_sekarang = date('Y');
        $bulan_sekarang = date('m');
        $point = '';
        $where = '';
