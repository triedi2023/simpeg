...
$pdf->Cell(0,6,'Daftar Urut Kepangkatan',0,1,'C');
...
// content
$pdf->SetWidths(array(10,40,30,105,30,30,30));
$pdf->HeaderTable(['No','Nama / NIP / Tgl Lahir / Umur','Gol / Ruang / TMT','Jabatan / TMT Jabatan / TMT Eselon','MK Total (Tahun Bulan)','Diklat PIM / Angkatan / Tahun','Pendidikan']);
$t = 1;
foreach ($model as $val):
    $nama = ((!empty($val['GELAR_DEPAN'])) ? $val['GELAR_DEPAN'] . " " : "") . ($val['NAMA']) . ((!empty($val['GELAR_BLKG'])) ? " " . $val['GELAR_BLKG'] : '');
    $pdf->Row([$t,$nama."\n".$val['NIPNEW']."\n".$val['TGLLAHIR']."\n".$val['UMUR'],$val['PANGKAT']." (".$val['GOLONGAN_RUANG'].") "."\n".$val['TMT_GOL'],$val['N_JABATAN']."\n".$val['TMT_JABATAN']."\n".$val['TMT_ESELON'],$val['MK_THN_TOTAL']." Tahun ".$val['MK_BLN_TOTAL']." Bulan",$val['NAMA_JENJANG']."\n".$val['ANGKATAN_DIKLAT']."\n".$val['THN_DIKLAT'],$val['TINGKAT_PENDIDIKAN']]);
    $t++;
endforeach;
