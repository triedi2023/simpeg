...
class Master_pegawai_cpns extends CI_Controller {

    private $data;

    public function __construct() {
        parent::__construct();
        $this->load->model(array('master_pegawai/master_pegawai_cpns_model', 'master_pegawai/master_pegawai_pendidikan_model', 'master_pegawai/master_pegawai_pangkat_model', 'list_model', 'master_pegawai/master_pegawai_model', 'master_pegawai/master_pegawai_jabatan_model'));
    ...
    public function index() {
        ...
        $this->data['data_jabatan'] = $this->master_pegawai_cpns_model->get_by_pegawai_jabatan_cpns($this->data['id'], (isset($this->data['data_cpns']['TMT_CPNS2']) ? $this->data['data_cpns']['TMT_CPNS2'] : NULL), isset($this->data['data_cpns']['NO_SK']) ? $this->data['data_cpns']['NO_SK'] : '', isset($this->data['data_cpns']['TGL_SK2']) ? $this->data['data_cpns']['TGL_SK2'] : '');
    ...
    public function ubah_proses() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $post = [
                ...
                "KDU4" => $kdu4,
                "KDU5" => $kdu5,
                "UNITKERJA_NOKODEREF" => ltrim(rtrim($this->input->post('unitkerjanokoderef', TRUE))),
            ...
            if ($this->master_pegawai_cpns_model->update($post, $tanggal, $this->input->get('id'))) {
                ...
                if ($this->input->post('pangkat', TRUE) != "") {
                    ...
                    if ($this->master_pegawai_jabatan_model->count_is_pns($this->input->get('id'), $this->input->post('no_sk', TRUE), $this->input->post('tmt_cpns', TRUE), $this->input->post('tgl_sk', TRUE)) < 1) {
                        ...
                        if (empty($_POST['jabatan_id']) || strlen($_POST['jabatan_id']) > 4) {
                            $nmjabatan = ltrim(rtrim($this->input->post('jabatan_nokoderef', TRUE)))." - ";
                        } else {
                        ...
                        $post = [
                            "TRESELON_ID" => trim($this->input->post('eselon', TRUE)),
                        ...
                        $tanggal = [
                            "TMT_JABATAN" => datepickertodb(trim($this->input->post('tmt_cpns', TRUE))),
                            "TGL_SK" => datepickertodb(trim($this->input->post('tgl_sk', TRUE))),
                        ];
                        ...
                        $model = $this->master_pegawai_jabatan_model->count_is_cpns($this->input->get('id'),ltrim(rtrim($this->input->post('no_sk', TRUE))),trim($this->input->post('tmt_cpns', TRUE)),trim($this->input->post('tgl_sk', TRUE)));
                ...
                $jabatan_mutakhir = $this->master_pegawai_jabatan_model->jabatan_mutakhir($this->input->get('id'));
                
                echo json_encode(['status' => 1, 'cu' => 'di-ubah', 'success' => 'Record updated successfully.', 
                'jabatan' => (isset($jabatan_mutakhir) && !empty($jabatan_mutakhir['N_JABATAN'])) ? $jabatan_mutakhir['N_JABATAN'] : '-',
                'pangkat'=>$pangkatterahir]);
            } else {
