...
class Master_pegawai_diklat_prajabatan extends CI_Controller {

    private $data;

    public function __construct() {
        parent::__construct();
        $this->load->model(array('master_pegawai/master_pegawai_diklat_prajabatan_model','list_model','master_pegawai/master_pegawai_model'));
        $this->load->helper(array('form','app_helper'));
        $this->data['title_utama'] = 'Diklat Prajabatan';
    }

    public function index() {
        $this->data['id'] = $this->input->get('id');
        $nippegawai = $this->master_pegawai_model->get_by_id_select($this->data['id'],"NIPNEW");
        $this->load->view('master_pegawai/diklat_prajabatan/index', $this->data);
    }
    ...
    public function tambah_form() {
        if ($this->input->is_ajax_request()) {
            ...
            $this->load->view("master_pegawai/diklat_prajabatan/form", $this->data);
        } else {
            redirect('/master_pegawai');
        }
    }
    ...
    public function tambah_proses() {
        ...
        $this->form_validation->set_rules('nama_diklat', 'Nama Diklat', 'required|min_length[1]|max_length[255]');
        $this->form_validation->set_rules('negara', 'Negara', 'required|min_length[1]|max_length[3]|is_natural_no_zero|trim');
        $this->form_validation->set_rules('jpl', 'JPL', 'min_length[1]|max_length[5]|is_natural_no_zero|trim');
        $this->form_validation->set_rules('penyelenggara', 'Penyelenggara', 'min_length[1]|max_length[300]');
    ...
    public function ubah_form() {
        if ($this->input->is_ajax_request()) {
            ...
            $this->data['model'] = $this->master_pegawai_diklat_prajabatan_model->get_by_id($this->input->get('id'));
            $this->data['title_form'] = "Ubah";
            $this->load->view("master_pegawai/diklat_prajabatan/form", $this->data);
        } else {
            redirect('/master_pegawai');
    ...
    public function ubah_proses() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $model = $this->master_pegawai_diklat_prajabatan_model->get_by_id($this->input->get('kode'));
            
            if ($this->master_pegawai_diklat_prajabatan_model->update($post,$tanggal,$this->input->get('kode'))) {
                $insert_id = $model['ID'];
    ...
    public function ajax_list() {
        ...
        $id = $this->input->get('id');
        $list = $this->master_pegawai_diklat_prajabatan_model->get_datatables($id);
        $data = array();
        $delete = '';
        $no = ($this->input->post('start')) ? $this->input->post('start') : 0;
        foreach ($list as $val) {
            $file = '';
            if (!empty($val->DOC_PRAJABATAN) && $val->DOC_PRAJABATAN != "") {
                $file = '<a href="javascript:;" class="btn btn-xs green-haze popupfull" data-url="'. site_url('master_pegawai/master_pegawai_diklat_prajabatan/view_dokumen?id='.$val->ID).'" title="Lihat Dokumen"><i class="fa fa-file-pdf-o"></i></a>';
            }
            if ($this->session->userdata('idgroup') != '' && $this->session->userdata('idgroup') != '3') {
                $delete = '<a href="javascript:;" class="hapusdataperrowlistdetailpegawai btn btn-xs red" data-url="'. site_url('master_pegawai/master_pegawai_diklat_prajabatan/hapus_data').'" data-id="'. $val->ID.'" title="Hapus Data"><i class="fa fa-trash"></i></a>';
            }
            ...
            $row[] = $val->JPL;
            $row[] = $val->NO_STTPP;
            $row[] = $file.'<a href="javascript:;" data-url="'. site_url('master_pegawai/master_pegawai_diklat_prajabatan/ubah_form?id='.$val->ID).'" class="btndefaultshowtambahubahdetailpegawai btn btn-xs yellow-saffron" title="'.($this->session->userdata('idgroup') != '' && $this->session->userdata('idgroup') == '3' ? 'Lihat Data' : 'Ubah Data').'"><i class="fa fa-edit"></i></a>'.$delete.'<a href="javascript:;" class="popuplarge btn btn-xs grey-cascade" data-url="'. site_url('master_pegawai/master_pegawai_diklat_prajabatan/view_info?id='.$val->ID).'" data-id="'. $val->ID.'" title="Info Data"><i class="fa fa-info-circle"></i></a>';
        ...
        $output = array(
            ...
            "recordsTotal" => $this->master_pegawai_diklat_prajabatan_model->count_all(),
            "recordsFiltered" => $this->master_pegawai_diklat_prajabatan_model->count_filtered(),
            "data" => $data,
        );
    ...
    public function hapus_data() {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post('id')) {
                $model = $this->master_pegawai_diklat_prajabatan_model->get_by_id($this->input->post('id'));
                $data_pegawai = $this->master_pegawai_model->get_by_id_select($model['TMPEGAWAI_ID'],"NIP,NIPNEW");
                if ($this->master_pegawai_diklat_prajabatan_model->hapus($this->input->post('id'))) {
                    if (!empty($model['DOC_PRAJABATAN']) && file_exists($this->config->item('uploadpath')."doc_pegawai/".preg_replace('/[^A-Za-z0-9\-]/', '-', trim($data_pegawai['NIP']))."/".$model['DOC_PRAJABATAN'])) {
                        unlink($this->config->item('uploadpath')."doc_pegawai/".preg_replace('/[^A-Za-z0-9\-]/', '-', trim($data_pegawai['NIP']))."/".$model['DOC_PRAJABATAN']);
                    }
    ...
    public function unique_edit() {
        $model = $this->master_pegawai_diklat_prajabatan_model->get_unique_nama_by_id($this->input->get('id'),$this->input->post('gol_darah'));
        if ($model > 0) {
    ...
    public function view_dokumen() {
        $model = $this->master_pegawai_diklat_prajabatan_model->get_dokumen_by_id($this->input->get('id'));
        $this->data['file'] = '';
        if (isset($model['NIP']) && $model['NIP'] != "") {
            if (file_exists($this->config->item('uploadpath')."doc_pegawai/".preg_replace('/[^A-Za-z0-9\-]/', '-', $model['NIP'])."/".$model['DOC_PRAJABATAN'])) {
                $this->data['file'] = base_url()."_uploads/doc_pegawai/".preg_replace('/[^A-Za-z0-9\-]/', '-', $model['NIP'])."/".$model['DOC_PRAJABATAN'];
            }
        }
    ...
    public function view_info() {
        $this->data['model'] = $this->master_pegawai_diklat_prajabatan_model->get_account_by_id($this->input->get('id'));
        $this->load->view('master_pegawai/diklat_prajabatan/info', $this->data);
    }
