...
class Master_pegawai_penilaian_tpa extends CI_Controller {

    private $data;

    public function __construct() {
        parent::__construct();
        $this->load->model(array('master_pegawai/master_pegawai_penilaian_tpa_model','list_model','master_pegawai/master_pegawai_model'));
        $this->load->helper(array('form','app_helper'));
        $this->data['title_utama'] = 'Penilaian TPA Pegawai';
    }

    public function index() {
        $this->data['id'] = $this->input->get('id');
        $this->load->view('master_pegawai/penilaian_tpa/index', $this->data);
    }
    ...
    public function tambah_form() {
        if ($this->input->is_ajax_request()) {
            ...
            $this->load->view("master_pegawai/penilaian_tpa/form", $this->data);
        } else {
            redirect('/master_pegawai');
        }
    }
    ...
    public function tambah_proses() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $post = [
                ...
                "LEMBAGA" => rtrim(ltrim($this->input->post('lembaga',TRUE))),
            ...
            if ($insert = $this->master_pegawai_penilaian_tpa_model->insert($post,$tanggal)) {
    ...
    public function ubah_form() {
        if ($this->input->is_ajax_request()) {
            ...
            $this->data['model'] = $this->master_pegawai_penilaian_tpa_model->get_by_id($this->input->get('id'));
            $this->data['id'] = $id;
    ...
    public function ubah_proses() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->master_pegawai_penilaian_tpa_model->update($post,$tanggal,$this->input->get('kode'))) {
    ...
    public function ajax_list() {
        ...
        $id = $this->input->get('id');
        $list = $this->master_pegawai_penilaian_tpa_model->get_datatables($id);
        $data = array();
        ...
        $output = array(
            ...
            "recordsTotal" => $this->master_pegawai_penilaian_tpa_model->count_all(),
            "recordsFiltered" => $this->master_pegawai_penilaian_tpa_model->count_filtered(),
            "data" => $data,
        );
    ...
    public function hapus_data() {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post('id')) {
                if ($this->master_pegawai_penilaian_tpa_model->hapus($this->input->post('id'))) {
    ...
    public function unique_edit() {
        $model = $this->master_pegawai_penilaian_tpa_model->get_unique_nama_by_id($this->input->get('id'),$this->input->post('gol_darah'));
        if ($model > 0) {
