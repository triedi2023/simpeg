...
class Master_pegawai_diklat_lain_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_DIKLAT_LAIN";
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_DIKLAT_LAIN.*, to_char(TGL_STTPP,'DD/MM/YYYY') as TGL_STTPP2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_DIKLATLAIN");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT THPEGAWAI_DIKLATLAIN_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_DIKLAT_LAIN", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_DIKLAT_LAIN", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_DIKLAT_LAIN");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
