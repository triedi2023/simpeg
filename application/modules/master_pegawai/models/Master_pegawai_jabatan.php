...
class Master_pegawai_jabatan extends CI_Controller {
    ...
    public function view_dokumen() {
        ...
        if (isset($model['NIP']) && $model['NIP'] != "") {
            if (file_exists($this->config->item('uploadpath')."doc_pegawai/".$model['NIP']."/".$model['DOC_SKJABATAN'])) {
                $this->data['file'] = base_url()."_uploads/doc_pegawai/".$model['NIP']."/".$model['DOC_SKJABATAN'];
            }
        }
