...
class Master_pegawai_penilaian_bahasa_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_BAHASA";
    private $column_order = array(null, 'JENIS_BAHASA','NAMA_BAHASA','NILAI'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_BAHASA.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_BAHASA.ID,JENIS_BAHASA,NAMA_BAHASA,NILAI");
        $this->db->from($this->tabel);
        $this->db->join("TR_JENIS_BAHASA","TR_JENIS_BAHASA.ID=TH_PEGAWAI_BAHASA.TRJENISBAHASA_ID","LEFT");
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_BAHASA.*, to_char(TGL_TEST,'DD/MM/YYYY') as TGL_TEST2, to_char(TGL_BERLAKU,'DD/MM/YYYY') as TGL_BERLAKU2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_BAHASA_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_BAHASA", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_BAHASA", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_BAHASA");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
