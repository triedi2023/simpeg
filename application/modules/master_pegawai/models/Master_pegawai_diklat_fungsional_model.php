...
class Master_pegawai_diklat_fungsional_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_DIKLAT_FUNGSIONAL";
    private $column_order = array(null, 'JENIS_DIKLAT_FUNGSIONAL','PENJENJANGAN_FUNGSIONAL','NAMA_PENJENJANGAN'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_DIKLAT_FUNGSIONAL.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_DIKLAT_FUNGSIONAL.ID,JENIS_DIKLAT_FUNGSIONAL,PENJENJANGAN_FUNGSIONAL,NAMA_PENJENJANGAN,DOC_DIKLATFUNGSIONAL");
        $this->db->from($this->tabel);
        $this->db->join("TR_JENIS_DIKLAT_FUNGSIONAL","TR_JENIS_DIKLAT_FUNGSIONAL.ID=TH_PEGAWAI_DIKLAT_FUNGSIONAL.TRJENISDIKLATFUNGSIONAL_ID","LEFT");
        $this->db->join("TR_PENJENJANGAN_FUNGSIONAL","TR_PENJENJANGAN_FUNGSIONAL.ID=TH_PEGAWAI_DIKLAT_FUNGSIONAL.TRPENJENJANGANFUNGSIONAL_ID","LEFT");
        $this->db->join("TR_NAMA_PENJENJANGAN","TR_NAMA_PENJENJANGAN.ID=TH_PEGAWAI_DIKLAT_FUNGSIONAL.TRNAMAPENJENJANGAN_ID","LEFT");
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_DIKLAT_FUNGSIONAL.*, to_char(TGL_STTPP,'DD/MM/YYYY') as TGL_STTPP2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_DIKLATFUNGSIONAL");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT THPEGAWAIDIKLATFUNGSIONAL_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_DIKLAT_FUNGSIONAL", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_DIKLAT_FUNGSIONAL", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_DIKLAT_FUNGSIONAL");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
