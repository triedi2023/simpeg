...
class Master_pegawai_diklat_penjenjangan_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_DIKLAT_PENJENJANGAN";
    private $column_order = array(null, 'NAMA_JENJANG','THN_DIKLAT','JPL'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_DIKLAT_PENJENJANGAN.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_DIKLAT_PENJENJANGAN.ID,NAMA_JENJANG,THN_DIKLAT,JPL,NO_STTPP,TGL_STTPP,DOC_PENJENJANGAN");
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_DIKLAT_PENJENJANGAN.*, to_char(TGL_STTPP,'DD/MM/YYYY') as TGL_STTPP2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_namadiklat_by_id($id) {
        $this->db->select("NAMA_JENJANG");
        $this->db->from("TR_TINGKAT_DIKLAT_KEPEMIMPINAN");
        $this->db->where('ID', $id);
        $query = $this->db->get();
        return $query->row_array()['NAMA_JENJANG'];
    }
    
    function get_diklat_by_idbkn($id) {
        $this->db->select("ID");
        $this->db->from("TR_TINGKAT_DIKLAT_KEPEMIMPINAN");
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_PENJENJANGAN");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT THPEGAWAIDIKLATPENJENJANGAN_S.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_DIKLAT_PENJENJANGAN", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_DIKLAT_PENJENJANGAN", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_DIKLAT_PENJENJANGAN");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
