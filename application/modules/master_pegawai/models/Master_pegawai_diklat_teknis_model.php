...
class Master_pegawai_diklat_teknis_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_DIKLAT_TEKNIS";
    private $column_order = array(null, 'NAMA_KELOMPOK','NAMA_DIKLAT', 'TEMPAT_KEGIATAN','KETERANGAN'); //set column field database for datatable orderable
    private $column_search = array('NAMA_NEGARA', 'STATUS'); //set column field database for datatable searchable 
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_DIKLAT_TEKNIS.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_DIKLAT_TEKNIS.ID,NAMA_KELOMPOK,TEMPAT_KEGIATAN,TR_DIKLAT_TEKNIS.NAMA_DIKLAT,KETERANGAN,DOC_TEKNIS");
        $this->db->from($this->tabel);
        $this->db->join("TR_KELOMPOK_DKLT_TEKNIS","TR_KELOMPOK_DKLT_TEKNIS.ID=TH_PEGAWAI_DIKLAT_TEKNIS.TRKELOMPOKDKLTTEKNIS_ID","LEFT");
        $this->db->join("TR_DIKLAT_TEKNIS","TR_DIKLAT_TEKNIS.ID=TH_PEGAWAI_DIKLAT_TEKNIS.TRDIKLATTEKNIS_ID","LEFT");
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_DIKLAT_TEKNIS.*, to_char(TGL_STTPP,'DD/MM/YYYY') as TGL_STTPP2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_TEKNIS");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_DIKLAT_TEKNIS_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_DIKLAT_TEKNIS", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_DIKLAT_TEKNIS", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_DIKLAT_TEKNIS");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
