...
class Master_pegawai_custom_model extends CI_Model {
        ...
        if ($this->session->get_userdata()['idgroup'] == 3) {
            ...
            if (!isset($_POST['pegawaibaru']) || $this->input->post('pegawaibaru') == "false") {
                ...
                if ($_POST['order'][0]['column'] == 0) {
                    $select = ", ROW_NUMBER() OVER(ORDER BY VPJM.TRESELON_ID,VPJM.TRLOKASI_ID, VPJM.KDU1, VPJM.KDU2, VPJM.KDU3, VPJM.KDU4, VPJM.KDU5) AS rn ";
                }
                
                $this->db->select("TP.ID,TP.GELAR_DEPAN,TP.NAMA,TP.GELAR_BLKG,TP.NIPNEW,TRG.TRSTATUSKEPEGAWAIAN_ID,TRG.GOLONGAN,TRG.PANGKAT,TO_CHAR(VPPM.TMT_GOL,'DD/MM/YYYY') AS TMT_GOL,VPJM.N_JABATAN,TO_CHAR(VPJM.TMT_JABATAN, 'DD/MM/YYYY') AS TMT_JABATAN".$select);
                $this->db->join("V_PEGAWAI_PANGKAT_MUTAKHIR VPPM", "VPPM.TMPEGAWAI_ID=TP.ID", "LEFT");
                $this->db->join("TR_GOLONGAN TRG", "TRG.ID=VPPM.TRGOLONGAN_ID", "LEFT");
                $this->db->join("V_PEGAWAI_JABATAN_MUTAKHIR VPJM", "VPJM.TMPEGAWAI_ID=TP.ID", "LEFT");
    ...
    public function jabatan_mutakhir($id) {
        return $this->db->query("SELECT N_JABATAN,TRE.ESELON FROM V_PEGAWAI_JABATAN_MUTAKHIR VPJM LEFT JOIN TR_ESELON TRE ON (TRE.ID=VPJM.TRESELON_ID) WHERE VPJM.TMPEGAWAI_ID=?", [$id])->row_array();
    }
    ...
    public function get_datatables_query_cetak($tipe = "") {
        ...
        if (!isset($_POST['pegawaibaru']) || $this->input->get('pegawaibaru') == "false") {
            if ($tipe == 1) {
                $this->db->where('VPJM.TRESELON_ID', (isset($this->session->userdata('config')['kode_fungsional_tertentu']) && $this->session->userdata('config')['kode_fungsional_tertentu'] != "") ? $this->session->userdata('config')['kode_fungsional_tertentu'] : '0');
            } elseif ($tipe == 2) {
                $this->db->where('VPJM.TRESELON_ID <>', (isset($this->session->userdata('config')['kode_fungsional_tertentu']) && $this->session->userdata('config')['kode_fungsional_tertentu'] != "") ? $this->session->userdata('config')['kode_fungsional_tertentu'] : '0');
            ...
            if ($this->input->get('statpeg_id')) {
                $this->db->where('TRG.TRSTATUSKEPEGAWAIAN_ID', trim($this->input->get('statpeg_id', TRUE)));
            }

            if ($this->input->get('gol_id')) {
        ...
        $this->db->order_by("VPJM.TRESELON_ID ASC,VPJM.TRLOKASI_ID ASC,VPJM.KDU1 ASC,VPJM.KDU2 ASC,VPJM.KDU3 ASC,VPJM.KDU4 ASC,VPJM.KDU5 ASC");
        $query = $this->db->get();
        return $query->result();
    }
