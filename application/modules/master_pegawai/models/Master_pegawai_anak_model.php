...
class Master_pegawai_anak_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_ANAK";
    private $column_order = array(null, 'STATUS_ANAK', 'NAMA', 'SEX', 'TGL_LAHIR', 'PEKERJAAN'); //set column field database for datatable orderable
    private $order = array('TGL_LAHIR' => 'ASC'); // default order 
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_ANAK.STATUS', $this->input->post('status'));
        }
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_ANAK.*, to_char(TGL_LAHIR,'DD/MM/YYYY') as TGL_LAHIR2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_AKTEANAK");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_ANAK_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_ANAK", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_ANAK", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_ANAK");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
