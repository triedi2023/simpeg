...
class Master_pegawai_seminar_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_SEMINAR";
    private $column_order = array(null, 'JENIS_KEGIATAN', 'NAMA_KEGIATAN', 'NAMA_NEGARA', 'TEMPAT', 'PENYELENGGARA','JENIS_PEMBIAYAAN','TAHUN'); //set column field database for datatable orderable
    private $order = array('TR_JENIS_KEGIATAN.ID' => 'ASC'); // default order 
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_SEMINAR.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_SEMINAR.ID,JENIS_KEGIATAN,NAMA_KEGIATAN,NAMA_NEGARA,TEMPAT,PENYELENGGARA,JENIS_PEMBIAYAAN,BULAN,TAHUN,DOC_SERTIFIKAT");
        $this->db->join("TR_JENIS_PEMBIAYAAN", "(TR_JENIS_PEMBIAYAAN.ID=TH_PEGAWAI_SEMINAR.TRJENISPEMBIAYAAN_ID)", "LEFT");
        $this->db->join("TR_JENIS_KEGIATAN", "(TR_JENIS_KEGIATAN.ID=TH_PEGAWAI_SEMINAR.TRJENISKEGIATAN_ID)", "LEFT");
        $this->db->join("TR_NEGARA", "(TR_NEGARA.ID=TH_PEGAWAI_SEMINAR.TRNEGARA_ID)", "LEFT");
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_SEMINAR.*, to_char(TGL_MULAI,'DD/MM/YYYY') as TGL_MULAI2, to_char(TGL_SELESAI,'DD/MM/YYYY') as TGL_SELESAI2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
        return $query->row_array();
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_SEMINAR_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        if (isset($tanggal['TGL_SELESAI']) && !empty($tanggal['TGL_SELESAI'])) {
            $this->db->set('TGL_SELESAI', "TO_DATE('" . $tanggal['TGL_SELESAI'] . "','YYYY-MM-DD')", FALSE);
        }
        $this->db->insert("TH_PEGAWAI_SEMINAR", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_SEMINAR", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_SEMINAR");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
