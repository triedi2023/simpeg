...
class Master_pegawai_ak_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_AK";
    private $column_order = array(null, 'TAHUN_KREDIT', 'JABATAN', 'AK_UTAMA', 'AK_PENUNJANG', 'AK_JUMLAH'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_AK.STATUS', $this->input->post('status'));
        }
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_AK.*, to_char(PERIODE_AWAL,'DD/MM/YYYY') as PERIODE_AWAL2, to_char(PERIODE_AKHIR,'DD/MM/YYYY') as PERIODE_AKHIR2, to_char(TGL_SK,'DD/MM/YYYY') as TGL_SK2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_AK");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_AK_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        if (isset($tanggal['PERIODE_AWAL']) && !empty($tanggal['PERIODE_AWAL'])) {
            $this->db->set('PERIODE_AWAL', "TO_DATE('" . $tanggal['PERIODE_AWAL'] . "','YYYY-MM-DD')", FALSE);
        }
        if (isset($tanggal['PERIODE_AKHIR']) && !empty($tanggal['PERIODE_AKHIR'])) {
            $this->db->set('PERIODE_AKHIR', "TO_DATE('" . $tanggal['PERIODE_AKHIR'] . "','YYYY-MM-DD')", FALSE);
        }
        if (isset($tanggal['TGL_SK']) && !empty($tanggal['TGL_SK'])) {
            $this->db->set('TGL_SK', "TO_DATE('" . $tanggal['TGL_SK'] . "','YYYY-MM-DD')", FALSE);
        }
        $this->db->insert("TH_PEGAWAI_AK", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_AK", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function list_jabatan_pegawai($id) {
        $this->db->select("N_JABATAN AS NAMA,TRJABATAN_ID AS ID");
