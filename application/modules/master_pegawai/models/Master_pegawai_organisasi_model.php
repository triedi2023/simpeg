...
class Master_pegawai_organisasi_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_ORGANISASI";
    private $column_order = array(null, 'JENIS_ORGANISASI', 'NAMA_ORG', 'JABATAN_ORG', 'TEMPAT_ORG', 'THN_TERDAFTAR','THN_SELESAI'); //set column field database for datatable orderable
    private $order = array('TH_PEGAWAI_ORGANISASI.THN_TERDAFTAR' => 'ASC'); // default order 
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_ORGANISASI.STATUS', $this->input->post('status'));
        }

        $this->db->select("TH_PEGAWAI_ORGANISASI.ID,JENIS_ORGANISASI,NAMA_ORG,JABATAN_ORG,TEMPAT_ORG,THN_TERDAFTAR,THN_SELESAI,DOC_SERTIFIKAT");
        $this->db->join("TR_JENIS_ORGANISASI", "(TR_JENIS_ORGANISASI.ID=TH_PEGAWAI_ORGANISASI.TRJENISORGANISASI_ID)", "LEFT");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_ORGANISASI_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array()) {
        ...
        $this->db->insert("TH_PEGAWAI_ORGANISASI", $var);
    ...
    public function update($var = array(), $id) {
        ...
        $this->db->set('UPDATED_DATE', "TO_DATE('" . date("Y-m-d H:i:s") . "','YYYY-MM-DD HH24:MI:SS')", FALSE);
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_ORGANISASI", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_ORGANISASI");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
