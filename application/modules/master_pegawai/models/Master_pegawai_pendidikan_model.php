...
class Master_pegawai_pendidikan_model extends CI_Model {

    private $tabel = "TH_PEGAWAI_PENDIDIKAN";
    private $column_order = array(null, 'TINGKAT_PENDIDIKAN', 'NAMA_LBGPDK', 'NAMA_FAKULTAS', 'NAMA_JURUSAN', 'NO_STTB', 'THN_LULUS'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('status')) {
            $this->db->where('TH_PEGAWAI_PENDIDIKAN.STATUS', $this->input->post('status'));
        }
    ...
    function get_by_id($id) {
        $this->db->select("TH_PEGAWAI_PENDIDIKAN.*, to_char(TGL_IJAZAH,'DD/MM/YYYY') as TGL_IJAZAH2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
    ...
    function get_tingkatpendidikan_bkn($id) {
        $this->db->select("ID");
        $this->db->from("TR_TINGKAT_PENDIDIKAN");
        $this->db->like('GROUP_ID_BKN', $id);
        $query = $this->db->get();
        return $query->row_array();
    }
    
    function get_dokumen_by_id($id) {
        $this->db->select("TP.NIP, THPP.DOC_IJASAH");
    ...
    private function next_val_id() {
        return $this->db->query("SELECT TH_PEGAWAI_PENDIDIKAN_SEQ.NEXTVAL AS NEXT_ID FROM DUAL")->row_array();
    }
    ...
    public function insert($var = array(), $tanggal = array()) {
        ...
        if (isset($tanggal['TGL_IJAZAH']) && !empty($tanggal['TGL_IJAZAH'])) {
            $this->db->set('TGL_IJAZAH', "TO_DATE('" . $tanggal['TGL_IJAZAH'] . "','YYYY-MM-DD')", FALSE);
        }
        $this->db->insert("TH_PEGAWAI_PENDIDIKAN", $var);
    ...
    public function update($var = array(), $tanggal = array(), $id) {
        ...
        $this->db->where('ID', $id);
        $this->db->update("TH_PEGAWAI_PENDIDIKAN", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
    ...
    public function hapus($id) {
        $this->db->trans_begin();
        $this->db->where('ID', $id);
        $this->db->delete("TH_PEGAWAI_PENDIDIKAN");
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
