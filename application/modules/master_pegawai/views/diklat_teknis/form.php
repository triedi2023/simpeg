<div class="col-md-12">
    <div class="portlet box blue">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-gift"></i><?php echo $title_form; ?> Data Diklat Teknis 
            </div>
            <div class="tools">&nbsp;</div>
        </div>
        <div class="portlet-body form">
            <?php echo form_open("://", ["class" => "formdetailpegawaiwithlist form-horizontal", 'data-url' => !isset($model['ID']) ? site_url('master_pegawai/master_pegawai_diklat_teknis/tambah_proses?id=' . $id) : site_url('master_pegawai/master_pegawai_diklat_teknis/ubah_proses?id=' . $model['TMPEGAWAI_ID'] . "&kode=" . $model['ID'])]); ?>
            <div class="form-body">
                        ...
                        <select name="kelompok" id="field_cr_kelompok" class="form-control" style="width: 100%">
                            ...
                            <?php if (isset($list_kelompok_diklat_teknis)): ?>
                                <?php foreach ($list_kelompok_diklat_teknis as $val): ?>
                                    <?php
                                    $selec = '';
                                    if ($val['ID'] == $model['TRKELOMPOKDKLTTEKNIS_ID'])
                                        $selec = 'selected=""';
                                    ?>
                ...
                <div class="form-group">
                    <label class="control-label col-md-3">Kelompok Diklat <span class="required" aria-required="true"> * </span></label>
                    <div class="col-md-7">
                        <select name="nama_diklat" id="field_cr_nama_diklat" class="form-control" style="width: 100%">
                            <option value="">- Pilih Kelompok Diklat -</option>
                            <?php if (isset($list_diklat_teknis)): ?>
                                <?php foreach ($list_diklat_teknis as $val): ?>
                                    <?php
                                    $selec = '';
                                    if ($val['ID'] == $model['TRDIKLATTEKNIS_ID'])
                                        $selec = 'selected=""';
                                    ?>
                                    <option <?= $selec ?> value="<?= $val['ID'] ?>"><?= $val['NAMA'] ?></option>
                                <?php endforeach ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-7">
                        <textarea name="keterangan" id="field_cr_keterangan" class="form-control" placeholder="Nama Diklat"><?php echo isset($model['KETERANGAN']) ? $model['KETERANGAN'] : set_value('keterangan'); ?></textarea>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-7">
                        <input type="text" name="tempat_kegiatan" maxlength="150" id="field_cr_tempat_kegiatan" class="form-control" value="<?php echo isset($model['TEMPAT_KEGIATAN']) ? $model['TEMPAT_KEGIATAN'] : set_value('tempat_kegiatan'); ?>" placeholder="Tempat Kegiatan" />
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if (!empty($model['DOC_PENJENJANGAN']) && $model['DOC_PENJENJANGAN'] <> '') { ?>
                        <div class="col-md-1">
                            <a href="javascript:;" data-url="<?php echo site_url('master_pegawai/master_pegawai_diklat_teknis/view_dokumen?id='.$model['ID']) ?>" title="Lihat Dokumen" class="btn btn-circle btn-icon-only green-haze popupfull">
...
<script>
    ...
    $("select#field_cr_nama_diklat").select2();
