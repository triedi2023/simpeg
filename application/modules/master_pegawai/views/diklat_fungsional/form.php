<div class="col-md-12">
    <div class="portlet box blue">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-gift"></i><?php echo $title_form; ?> Data Diklat Fungsional 
            </div>
            <div class="tools">&nbsp;</div>
        </div>
        <div class="portlet-body form">
            <?php echo form_open("://", ["class" => "formdetailpegawaiwithlist form-horizontal", 'data-url' => !isset($model['ID']) ? site_url('master_pegawai/master_pegawai_diklat_fungsional/tambah_proses?id=' . $id) : site_url('master_pegawai/master_pegawai_diklat_fungsional/ubah_proses?id=' . $model['TMPEGAWAI_ID'] . "&kode=" . $model['ID'])]); ?>
            <div class="form-body">
                
                <div class="form-group">
                    <label class="control-label col-md-3">Jenis Diklat <span class="required" aria-required="true"> * </span></label>
                    <div class="col-md-7">
                        <select name="jenis_diklat" id="field_cr_jenis_diklat" class="form-control" style="width: 100%">
                            <option value="">- Pilih Jenis Diklat -</option>
                            <?php if (isset($list_jenis_diklat_fungsional)): ?>
                                <?php foreach ($list_jenis_diklat_fungsional as $val): ?>
                                    <?php
                                    $selec = '';
                                    if (isset($model) && $val['ID'] == $model['TRJENISDIKLATFUNGSIONAL_ID'])
                                        $selec = 'selected=""';
                                    ?>
                ...
                <div class="form-group">
                    <label class="control-label col-md-3">Tingkat <span class="required" aria-required="true"> * </span></label>
                    <div class="col-md-7">
                        <select name="tingkat" id="field_cr_tingkat" class="form-control" style="width: 100%">
                            <option value="">- Pilih -</option>
                            <?php if (isset($list_penjenjangan_fungsional)): ?>
                                <?php foreach ($list_penjenjangan_fungsional as $val): ?>
                                    <?php
                                    $selec = '';
                                    if (isset($model) && $val['ID'] == $model['TRPENJENJANGANFUNGSIONAL_ID'])
                                        $selec = 'selected=""';
                                    ?>
                                    <option <?= $selec ?> value="<?= $val['ID'] ?>"><?= $val['NAMA'] ?></option>
                                <?php endforeach ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-7">
                        <select name="jenjang" id="field_cr_jenjang" class="form-control" style="width: 100%">
                            <?php if (isset($list_nama_penjenjangan)): ?>
                                <?php foreach ($list_nama_penjenjangan as $val): ?>
                                    <?php
                                    $selec = '';
                                    if (isset($model) && $val['ID'] == $model['TRNAMAPENJENJANGAN_ID'])
                                        $selec = 'selected=""';
                                    ?>
                                    <option <?= $selec ?> value="<?= $val['ID'] ?>"><?= $val['NAMA'] ?></option>
                                <?php endforeach ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-7">
                        <input type="text" name="penyelenggara" maxlength="128" id="field_cr_penyelenggara" class="form-control" value="<?php echo isset($model['PENYELENGGARA']) ? $model['PENYELENGGARA'] : set_value('penyelenggara'); ?>" placeholder="Penyelenggara" />
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if (!empty($model['DOC_PENJENJANGAN']) && $model['DOC_PENJENJANGAN'] <> '') { ?>
                        <div class="col-md-1">
                            <a href="javascript:;" data-url="<?php echo site_url('master_pegawai/master_pegawai_diklat_fungsional/view_dokumen?id='.$model['ID']) ?>" title="Lihat Dokumen" class="btn btn-circle btn-icon-only green-haze popupfull">
...
<script>
    $("select#field_cr_jenis_diklat").select2();
    $("select#field_cr_kelompok").select2();
    $("select#field_cr_tingkat").select2();
    $("select#field_cr_jenjang").select2();
