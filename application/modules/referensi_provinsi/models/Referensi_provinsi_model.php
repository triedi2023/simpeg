...
class Referensi_provinsi_model extends CI_Model {

    private $tabel = "TR_PROPINSI";
    private $column_order = array(null, 'NAMA_PROPINSI', 'STATUS'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query() {
        ...
        if ($this->input->post('propinsi')) {
            $this->db->like('lower(NAMA_PROPINSI)', strtolower($this->input->post('propinsi')));
        }
    ...
    public function next_val_id() {
        $sql = "WITH t(kode) AS (
                SELECT 1 as kode from dual
                    UNION ALL
                SELECT kode+1 FROM t WHERE to_number(kode) < (select max(to_number(ID)) from TR_PROPINSI)
            )
            SELECT MIN(to_number(KODE)) as KODE FROM t WHERE to_number(kode) NOT IN (SELECT to_number(ID) FROM TR_PROPINSI)";
        ...
        if ($query->row_object()->KODE <> "") {
            ...
            $maxquery = $this->db->query("SELECT MAX(to_number(ID)) as KODE FROM TR_PROPINSI");
    ...
    public function insert($var = array()) {
        ...
        $this->db->insert("TR_PROPINSI", $data);
    ...
    public function update($var = array(), $id) {
        ...
        $this->db->set('UPDATED_DATE', "TO_DATE('" . date("Y-m-d H:i:s") . "','YYYY-MM-DD HH24:MI:SS')", FALSE);
        $this->db->where('ID', $id);
        $this->db->update("TR_PROPINSI", $var);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        } else {
