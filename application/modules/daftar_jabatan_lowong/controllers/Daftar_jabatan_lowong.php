...
class Daftar_jabatan_lowong extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('daftar_jabatan_lowong/daftar_jabatan_lowong_model', 'list_model'));
        $this->load->helper(array('form', 'app_helper'));
        $this->data['plugin_js'] = ['assets/plugins/select2/js/select2.full.min.js'];
        $this->data['custom_js'] = ['daftar_jabatan_lowong/js', 'system/unitkerja_form-horizontal_filter_js'];
        $this->data['title_utama'] = 'Daftar Jabatan Lowong';
    }

    public function index() {
        $this->data['content'] = 'daftar_jabatan_lowong/index';
        $this->data['list_lokasi'] = json_encode($this->list_model->list_lokasi_tree('',(!empty($this->session->userdata('trlokasi_id')) && $this->session->userdata('idgroup') == 2 ? $this->session->userdata('trlokasi_id') : '')));
    ...
    public function pencarian_proses() {
        ...
        $this->data['model'] = $this->daftar_jabatan_lowong_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['struktur'] = $this->daftar_jabatan_lowong_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);

        $this->load->view("daftar_jabatan_lowong/_hasil", $this->data);
    }
    ...
    public function export_excel() {
        ...
        $model = $this->daftar_jabatan_lowong_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $struktur = $this->daftar_jabatan_lowong_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $jumlah = count($model);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Daftar Jabatan Kosong');
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('B' . $judultabel, "Jabatan")
                ->setCellValue('C' . $judultabel, "Lama Jabatan Kosong");
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabel . ':C' . $judultabel)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
        ...
        if ($model) {
            foreach ($model as $r2) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('B' . $j, $r2['NMJABATAN'])
                        ->setCellValue('C' . $j, $r2['LAMA_LOWONG']);
        ...
        $objPHPExcel->getActiveSheet()->getHeaderFooter()->setOddHeader('&L&G&C&HDAFTAR JABATAN LOWONG ' . $this->config->item('instansi_panjang'));
    ...
    public function export_pdf() {
        ...
        $this->data['model'] = $this->daftar_jabatan_lowong_model->get_data($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['struktur'] = $this->daftar_jabatan_lowong_model->get_struktur($lokasi_id, $kdu1, $kdu2, $kdu3, $kdu4, $kdu5);
        $this->data['content'] = 'daftar_jabatan_lowong/pdf';
