...
class Statistik_jabatan extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('statistik_jabatan/statistik_jabatan_model', 'list_model'));
        $this->load->helper(array('form', 'app_helper'));
        $this->data['plugin_js'] = array_merge(list_js_datatable(), ['assets/plugins/select2/js/select2.min.js']);
        $this->data['custom_js'] = ['statistik_jabatan/js'];
        $this->data['title_utama'] = 'Statistik Jabatan';
    }

    public function index() {
        $this->data['content'] = 'statistik_jabatan/index';
        $this->load->view('layouts/main', $this->data);
    }

    public function pencarian_proses() {
        $this->data['jenjang_fungsional'] = $this->statistik_jabatan_model->get_jenjang_fungsional();
        $this->load->view("statistik_jabatan/_hasil", $this->data);
    }

    public function export_pdf() {
        $this->data['data_komposisi'] = $this->statistik_jabatan_model->get_data();
        $this->data['content'] = 'statistik_jabatan/pdf';
    ...
    public function liststatistik() {
        ...
        $this->load->view("statistik_jabatan/liststatistik",$this->data);
    }
    ...
    public function ajax_liststatistik() {
        ...
        $this->load->model(['statistik_jabatan_detail_model']);
        $list = $this->statistik_jabatan_detail_model->get_datatables();
        $data = array();
        ...
        $output = array(
            ...
            "recordsTotal" => $this->statistik_jabatan_detail_model->count_all(),
            "recordsFiltered" => $this->statistik_jabatan_detail_model->count_filtered(),
            "data" => $data,
        );
    ...
    public function export_excel() {
        $model = $this->statistik_jabatan_model->get_jenjang_fungsional();
        $jumlah = count($model);
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('C' . $judultabel, "Jenjang ")
                ->setCellValue($alphabet[$jmldata+2] . $judultabel, "Jumlah")
                ->setCellValue($alphabet[$jmldata+3] . $judultabel, "Calon Rescuer")
                ->setCellValue($alphabet[$jmldata+4] . $judultabel, "Total");
        if ($model) {
            $urutsamping = 2;
            foreach ($model as $isinya) {
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$urutsamping] . $headertabel, ucwords(strtolower($isinya['TINGKAT_FUNGSIONAL'])));
                $urutsamping++;
            }
        }
        ...
        $objPHPExcel->getActiveSheet()->mergeCells('C' . $judultabel . ':'. $alphabet[$jmldata+1] . $judultabel);
        $objPHPExcel->getActiveSheet()->mergeCells($alphabet[$jmldata+2] . $judultabel . ':'. $alphabet[$jmldata+2] . $headertabel);
        $objPHPExcel->getActiveSheet()->mergeCells($alphabet[$jmldata+3] . $judultabel . ':'. $alphabet[$jmldata+3] . $headertabel);
        $objPHPExcel->getActiveSheet()->mergeCells($alphabet[$jmldata+4] . $judultabel . ':'. $alphabet[$jmldata+4] . $headertabel);
        if ($model) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    if ($model) {
                        ...
                        foreach ($model as $isinya) {
                            ...
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$urutsamping] . $j, $hasilnya['JMLPEGAWAI']);
                            
                            $urutsamping++;
                            $totaljenjang += $hasilnya['JMLPEGAWAI'];
                        }
                    }
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+2] . $j, $totaljenjang);
                    ...
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+3] . $j, $hasilnih['JMLPEGAWAI']);
                    $totalcalon += $hasilnih['JMLPEGAWAI'];
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+4] . $j, $totaljenjang + $hasilnih['JMLPEGAWAI']);
                $j++;
                $k++;
            }
        }
        ...
        if ($model) {
            ...
            foreach ($model as $val) {
                ...
                $hasilnya = $this->db->query($sqlnya)->row_array();
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$urutalpa] . ($headertabel+$k), $hasilnya['JMLPEGAWAI']);
                $hasiltotal += $hasilnya['JMLPEGAWAI'];
                $urutalpa++;
            }
        }
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+2] . ($headertabel+$k), $hasiltotal);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+3] . ($headertabel+$k), $totalcalon);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphabet[$jmldata+4] . ($headertabel+$k), $hasiltotal + $totalcalon);
        
        $objPHPExcel->getActiveSheet()->mergeCells('A' . ($headertabel + ($k)) . ':B' . ($headertabel + ($k)));
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabel . ':'.$alphabet[$jmldata+4] . ($headertabel + ($k)))->applyFromArray($styleArray);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabel . ':'.$alphabet[$jmldata+4] . $headertabel)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabel . ':'.$alphabet[$jmldata+4] . $headertabel)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
