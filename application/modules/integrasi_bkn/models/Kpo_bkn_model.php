...
class Kpo_bkn_model extends CI_Model {

    private $tabel = "TH_KPO_BKN";
    private $column_order = array(null, "NAMA", "NIPNEW", 'NO_SK', 'TGL_SK', "JENIS_KENAIKAN_PANGKAT", 'GOLONGAN', 'PANGKAT'); //set column field database for datatable orderable
    private $order = array('TMT_GOLONGAN' => 'DESC','TGL_SK' => 'DESC'); // default order 
    ...
    private function _get_datatables_query() {
        ...
        $this->db->select("TH_KPO_BKN.ID,NAMA,TP.NIPNEW,NO_SK,TO_CHAR(TGL_SK,'DD/MM/YYYY') as TGL_SK2,JENIS_KENAIKAN_PANGKAT,TRG.GOLONGAN,TO_CHAR(TMT_GOLONGAN,'DD/MM/YYYY') as TMT_GOLONGAN2,TRG.PANGKAT,TP.ID AS IDPEGAWAI");
        $this->db->from($this->tabel);
        $this->db->join("TR_GOLONGAN TRG","TRG.ID_BKN=TH_KPO_BKN.GOLONGAN_ID AND TRG.TRSTATUSKEPEGAWAIAN_ID='1'","LEFT JOIN");
        $this->db->join("TR_JENIS_KENAIKAN_PANGKAT TJK","TJK.ID_BKN=TH_KPO_BKN.JENISKP_ID","LEFT JOIN");
        $this->db->join("TM_PEGAWAI TP","TP.NIPNEW=TH_KPO_BKN.NIPBARU","JOIN");
        $i = 0;
    ...
    function get_by_id($id) {
        $this->db->select("TH_KPO_BKN.*, to_char(TMT_GOLONGAN,'DD/MM/YYYY') as TMT_GOLONGAN2,to_char(TGL_SK,'DD/MM/YYYY') as TGL_SK2");
        $this->db->from($this->tabel);
        $this->db->where('ID', $id);
        $query = $this->db->get();
