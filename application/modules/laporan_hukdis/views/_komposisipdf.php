...
$pdf->Cell(0, 6, 'Komposisi Daftar Hukuman Disiplin / Sanksi', 0, 1, 'C');
...
// content
$pdf->SetWidths(array(10, 227, 40));
$pdf->HeaderTable(['No', 'Jenis Hukuman', 'Jumlah']);
$t = 1;
foreach ($model as $val):
    $nama = ((!empty($val['GELAR_DEPAN'])) ? $val['GELAR_DEPAN'] . " " : "") . ($val['NAMA']) . ((!empty($val['GELAR_BLKG'])) ? " " . $val['GELAR_BLKG'] : '');
    $pdf->Row([$t, $val['NIPNEW'], $nama, $val['TKT_HUKUMAN_DISIPLIN'], $val['JENIS_HUKDIS'], $val['ALASAN_HKMN'], $val['TMT_HKMN']." S/D ".$val['AKHIR_HKMN'],$val['NO_SK']." ".$val['TGL_SK'],$val['NAMA_UNIT_KERJA']]);
    $t++;
endforeach;
