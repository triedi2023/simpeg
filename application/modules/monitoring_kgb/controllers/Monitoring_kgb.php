...
class Monitoring_kgb extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('monitoring_kgb/monitoring_kgb_model', 'list_model'));
        $this->load->helper(array('form', 'app_helper'));
        $this->data['plugin_js'] = ['assets/plugins/select2/js/select2.full.min.js'];
        $this->data['custom_js'] = ['monitoring_kgb/js', 'system/unitkerja_form-horizontal_filter_js'];
        $this->data['title_utama'] = 'Proyeksi KGB Reguler';
    }

    public function index() {
        $this->data['content'] = 'monitoring_kgb/index';
    ...
    public function pencarian_proses() {
        ...
        $bulan1 = (isset($_POST['bulan1']) && !empty($_POST['bulan1'])) ? trim($this->input->post('bulan1', TRUE)) : date('m');
        $tahun1 = (isset($_POST['tahun1']) && !empty($_POST['tahun1'])) ? trim($this->input->post('tahun1', TRUE)) : date('Y');
        $bulan2 = (isset($_POST['bulan2']) && !empty($_POST['bulan2'])) ? trim($this->input->post('bulan2', TRUE)) : date('m');
        $tahun2 = (isset($_POST['tahun2']) && !empty($_POST['tahun2'])) ? trim($this->input->post('tahun2', TRUE)) : date('Y')+1;
        
        $this->data['model'] = $this->monitoring_kgb_model->get_data($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$eselon_id,$bulan1,$tahun1,$bulan2,$tahun2);
        $this->data['struktur'] = $this->monitoring_kgb_model->get_struktur($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5);
        
        $this->load->view("monitoring_kgb/_hasil", $this->data);
    }
    ...
    public function export_excel() {
        ...
        $model = $this->monitoring_kgb_model->get_data($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$eselon_id,$bulan1,$tahun1,$bulan2,$tahun2);
        $struktur = $this->monitoring_kgb_model->get_struktur($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5);
        $jumlah = count($model);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Proyeksi KGB Reguler');
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('B' . $judultabellagi, "Nama / NIP / Lahir / Umur")
                ->setCellValue('C' . $judultabellagi, "Gol. Ruang & TMT")
                ->setCellValue('D' . $judultabellagi, "TMT CPNS / MKG / Gaji Pokok")
                ->setCellValue('F' . $judultabellagi, "Keterangan");
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabellagi . ':F' . ($judultabel + $jumlah))->applyFromArray($styleArray);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabellagi . ':F' . $judultabel)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $judultabellagi . ':F' . $judultabel)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $objPHPExcel->getActiveSheet()->mergeCells('D' . $judultabellagi . ':E' . $judultabellagi);
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('D' . $judultabel, "Lama")
                ->setCellValue('E' . $judultabel, "Baru");
        ...
        $objPHPExcel->getActiveSheet()->mergeCells('F' . $judultabellagi . ':F' . $judultabel);
        ...
        if ($model) {
            foreach ($model as $val) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('C' . $j, $val['PANGKAT']." (".$val['GOLONGAN_RUANG'].")"."\n".$val['TMT_GOL'])
                        ->setCellValue('D' . $j, $val['TMT_KGB']."\n".str_replace('-','',$val['MKG_LAMA'])."\nRp ".number_format($val['GAJI_LAMA'], 0))
                        ->setCellValue('E' . $j, $val['TMT_KGB_NEXT_CHAR']."\n".str_replace('-','',$val['MKG_BARU'])."\nRp ".number_format($val['GAJI_BARU'], 0))
                        ->setCellValue('F' . $j, 'a. Golongan Gaji = '.$val['GOLONGAN_RUANG']."\nb. Saat kenaikan gaji pokok berikutnya tanggal = ".$val['TMT_KGB_NEXT2']);
        ...
        $objPHPExcel->getActiveSheet()->getHeaderFooter()->setOddHeader('&L&G&C&HDAFTAR PROYEKSI KGB REGULER ' . $this->config->item('instansi_panjang'));
    ...
    public function export_pdf() {
        ...
        $this->data['model'] = $this->monitoring_kgb_model->get_data($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5,$gol_pangkat,$eselon_id,$bulan1,$tahun1,$bulan2,$tahun2);
        $this->data['struktur'] = $this->monitoring_kgb_model->get_struktur($lokasi_id,$kdu1,$kdu2,$kdu3,$kdu4,$kdu5);
        $this->data['content'] = 'monitoring_kgb/pdf';
