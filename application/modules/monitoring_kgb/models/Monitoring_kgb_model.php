...
class Monitoring_kgb_model extends CI_Model {
    ...
    function get_data($lok='2',$kdu1='00',$kdu2='00',$kdu3='000',$kdu4='000',$kdu5='00',$gol_pangkat="",$eselon="",$bulan="",$tahun='',$bulan1="",$tahun1='') {
        ...
        if (!empty($eselon) && $eselon != "") {
            ...
            $condition = array_merge($condition, [$eselon]);
        }
        
        if (!empty($bulan) && !empty($tahun) && !empty($bulan1) && !empty($tahun1)) {
            $where .= " AND to_char(TO_DATE(TMT_KGB_NEXT),'YYYY-MM-DD') BETWEEN ('$tahun-$bulan') AND ('$tahun1-$bulan1')";
        } else {
            $where .= " AND to_char(TO_DATE(TMT_KGB_NEXT),'YYYY-MM-DD') BETWEEN ('$tahun_sekarang-$bulan_sekarang') AND ('$tahun1-$bulan1')";
        }
        
        $sql = "SELECT * FROM V_MON_KGB WHERE 1=1 $where";
