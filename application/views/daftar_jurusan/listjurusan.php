<div class="modal-header bg-yellow-soft bg-font-yellow-soft" style="padding: 10px">
    ...
    <h4 class="modal-title bg-font-blue-madison font-lg sbold pull-left">Daftar Jurusan</h4>
</div>
<div class="modal-body">
    <div class="m-heading-1 border-yellow-haze m-bordered" id="divsearchpopup" style="display: none">
        <form class="form-inline popupformfilter" id="popupformpencarianjurusan" role="form" action="javascript:;">
            <div class="form-group">
                <label class="sr-only" for="nama_jur">Nama</label>
                <input type="nama_jur" class="form-control input-xlarge" id="nama_jur" placeholder="Cari berdasarkan Nama Jurusan">
            </div>
            <button type="submit" class="btn btn-default blue-madison">Cari</button>
        </form>
    </div>
    <div class="panel-success">
        <div class="table-container" data-url="<?php echo site_url('daftar_jurusan/ajax_listjurusan') ?>">
            <table class="table popuplistjurusan table table-striped table-bordered table-hover order-column dataTable no-footer">
                <thead>
...
<script>
    var datasearch = function (data) {
        data.nama_jurusan = $("input#nama_jur").val();
    }
    var $popuplistjurusan = $('table.popuplistjurusan').DataTable({
        "language": {
            "aria": {
        ...
        processing: true,
        ajax: {
            url: '<?php echo site_url('daftar_jurusan/ajax_listjurusan') ?>',
            type: 'POST',
            data: datasearch
        }
    });
