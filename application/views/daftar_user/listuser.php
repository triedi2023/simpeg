<div class="modal-header bg-yellow-soft bg-font-yellow-soft" style="padding: 10px">
    ...
    <h4 class="modal-title bg-font-blue-madison font-lg sbold pull-left">Daftar User</h4>
</div>
    ...
    <div class="m-heading-1 border-yellow-haze m-bordered" id="divsearchpopup" style="display: none">
        <form class="form-inline popupformfilter" role="form" action="javascript:;">
            <div class="form-group">
                <label class="sr-only" for="nama_username">Username</label>
                <input type="text" class="form-control input-xlarge" id="nama_username" name="username" placeholder="Username" />
            </div>
            <button type="submit" class="btn btn-default blue-madison">Cari</button>
        </form>
    </div>
    <div class="panel-success">
        <div class="table-container" data-url="<?php echo site_url('daftar_user/ajax_listuser') ?>">
...
//        data.setelementnya = "<?php // echo $setelementnya; ?>";
        data.nama_username = $("input#nama_username").val();
    }
    var $popuplistpegawai = $('table.popuplistpegawai').DataTable({
        ...
        processing: true,
        ajax: {
            url: '<?php echo site_url('daftar_user/ajax_listuser') ?>',
            type: 'POST',
            data: datasearch
        }
    });
