...
<div class="modal-body">
    <div class="m-heading-1 border-yellow-haze m-bordered" id="divsearchpopup" style="display: none">
        <form class="form-inline popupformfilter" name="popupformfilter" role="form" action="javascript:;">
            <div class="form-group">
                <label class="sr-only" for="nama_nip">NIP / Nama</label>
                <input type="nama_nip" class="form-control input-xlarge" id="nama_nip" placeholder="Cari berdasarkan NIP atau Nama">
            </div>
            <button type="submit" class="btn btn-default blue-madison">Cari</button>
        </form>
    </div>
    <div class="panel-success">
        <div class="table-container" data-url="<?php echo site_url('daftar_pegawai/ajax_listpegawai') ?>">
            <table class="table popuplistpegawai table table-striped table-bordered table-hover order-column dataTable no-footer">
                <thead>
                    <tr class="bg-yellow-haze bg-font-yellow-haze">
                        ...
                        <th style="width: 5%" class="text-center"> NIP </th>
...
<script>
    var datasearch = function (data) {
        data.setelementnya = "<?php echo $setelementnya; ?>";
    ...
    var $popuplistpegawai = $('table.popuplistpegawai').DataTable({
        ...
        processing: true,
        ajax: {
            url: '<?php echo site_url('daftar_pegawai/ajax_listpegawai') ?>',
            type: 'POST',
            data: datasearch
        }
    });
    $("div.iconsearchpopup").html('<a href="javascript:;" class="btnpopupsearch btn blue btn-sm btn-circle"><i class="fa fa fa-search"></i></a>');
