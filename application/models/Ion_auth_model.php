...
//        $this->db->update($this->tables['users'], $data, [$this->identity_column => $identity]);
    ...
    public function get_pegawai($id) {
        $query = $this->db->select("GELAR_DEPAN, NAMA, GELAR_BLKG, NAMA_GROUP, NIPNEW, SYSTEMGROUP_ID, VPJM.TRLOKASI_ID, VPJM.KDU1, VPJM.KDU2, VPJM.KDU3, VPJM.KDU4, VPJM.KDU5")
                ->where("SYSTEMUSER_ID", $id)
                ->from("SYSTEM_USER_GROUP SUG")->join("TM_PEGAWAI TMP", "TMP.ID=SUG.TMPEGAWAI_ID", 'JOIN')
                ->join("V_PEGAWAI_JABATAN_MUTAKHIR VPJM", "TMP.ID=VPJM.TMPEGAWAI_ID", 'JOIN')
                ->join("SYSTEM_GROUP SG", "SG.ID=SUG.SYSTEMGROUP_ID", 'JOIN')->get();
        $val = $query->row();
        $nama = ((!empty($val->GELAR_DEPAN)) ? $val->GELAR_DEPAN." ": "").($val->NAMA).((!empty($val->GELAR_BLKG)) ? " ".$val->GELAR_BLKG : '');
        
        return ['nama' => $nama, 'NIPNEW' => $val->NIPNEW, 'group' => $val->NAMA_GROUP, 'idgroup' => $val->SYSTEMGROUP_ID, 
        'trlokasi_id' => $val->TRLOKASI_ID,'kdu1' => $val->KDU1,'kdu2' => $val->KDU2,'kdu3' => $val->KDU3,'kdu4' => $val->KDU4,'kdu5' => $val->KDU5];
    }
    ...
    public function login($identity, $password="", $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column. ", EMAIL, LAST_LOGIN, ID")
                ->where($this->identity_column, $identity)
                ->get($this->tables['users']);
...
//        $this->set_error('login_unsuccessful');
    ...
    public function update_last_login($id) {
        ...
        $this->db->set('LAST_LOGIN', "TO_DATE('" . date("Y-m-d H:i:s") . "','YYYY-MM-DD HH24:MI:SS')", FALSE);
        $this->db->update($this->tables['users'], [], ['ID' => $id]);
