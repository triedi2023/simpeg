...
class Apikepegawaian_model extends CI_Model {
    ...
    public function get_data_anak($id) {
        $query = "SELECT sis.NAMA as \"statusanak\",to_char(TGL_LAHIR, 'DD/MM/YYYY') as \"tggl_lahir\",tsi.NAMA \"nama\",(case when SEX = 'L' then 'Laki-laki' when SEX='P' then 'Perempuan' else '' end) as \"jk\",
        TEMPAT_LHR \"tempat_lhr\",PEKERJAAN \"kerja\" FROM TH_PEGAWAI_ANAK tsi LEFT JOIN TR_STATUS_ANAK sis on (tsi.TRSTATUSANAK_ID=sis.ID) 
